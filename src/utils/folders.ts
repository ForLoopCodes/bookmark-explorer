...
export const getFolderPath = (folderId: string | undefined, folders: Folder[]): BreadcrumbItem[] => {
  if (!folderId) {
...
export const getFolderChildren = (parentId: string | undefined, folders: Folder[]): Folder[] => {
...
export const canDeleteFolder = (folderId: string, folders: Folder[]): boolean => {
  // Check if folder has any subfolders
  const hasSubfolders = folders.some(folder => folder.parentId === folderId);
  return !hasSubfolders;
};

export const deleteFolderRecursive = (folderId: string, folders: Folder[]): string[] => {
